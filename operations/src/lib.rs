...
pub fn demo(file: &PathBuf, user: &UserID, position: &Point3f) -> Result<(), DBError> {
    ...
    let wall_1 = Wall::new(position.clone(), position_2.clone(), width, height);
    let id_1 = wall_1.get_id().clone();
    let wall_2 = Wall::new(position_2.clone(), position_3.clone(), width, height);
    let id_2 = wall_2.get_id().clone();
    let wall_3 = Wall::new(position_3.clone(), position_4.clone(), width, height);
    let id_3 = wall_3.get_id().clone();
    let wall_4 = Wall::new(position_4.clone(), position.clone(), width, height);
    ...
    let dim_4 = Dimension::new(position_4.clone(), position.clone(), offset);
