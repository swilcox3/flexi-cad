...
pub struct Dimension {
    id: RefID,
    pub first: Geometry<RefPoint>,
    pub second: Geometry<RefPoint>,
...
impl Dimension {
    pub fn new(first: Point3f, second: Point3f, offset: WorldCoord) -> Dimension {
        ...
        Dimension {
            id,
            first: Geometry::new(RefPoint{pt: first}),
            second: Geometry::new(RefPoint{pt: second}),
            offset: offset,
        }
    }
}
...
impl Data for Dimension{
    ...
    fn update(&mut self) -> Result<UpdateMsg, DBError> {
        let perp = get_perp_2d(&self.first.pt, &self.second.pt);
        let line_1 = self.first.pt + perp * self.offset; 
        let line_2 = self.second.pt + perp * self.offset;
        ...
        let data = json!({
            ...
            "first": graphic_space(&self.first.pt),
            "first_off": graphic_space(&line_1),
            "second": graphic_space(&self.second.pt),
    ...
    fn get_data(&self, prop_name: &str) -> Result<serde_json::Value, DBError> {
        match prop_name {
            ...
            "First" => serde_json::to_value(&self.first.pt).map_err(error_other),
            "Second" => serde_json::to_value(&self.second.pt).map_err(error_other),
...
impl UpdateFromRefs for Dimension {
    ...
    fn get_associated_point(&self, index: PointIndex) -> Option<Point3f> {
        match index {
            0 => Some(self.first.pt),
            1 => Some(self.second.pt),
            _ => {
                None
            }
        }
